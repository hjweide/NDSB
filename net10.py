...
if __name__ == '__main__':
    ...
    trained_net_file = join(root, 'nets', 'net10.pickle')
    submission_file = join(root, 'submissions', 'submit_net_10.csv')
    ...
    if retrain:
        ...
        train_net(data, labels, net10, trained_net_file)
        plot_loss(net10, outfile=join(root, 'plots', 'net10_loss.png'))
    else:
        print('loading net from disk...')
        with open(trained_net_file, 'rb') as ifile:
            net10 = pickle.load(ifile)
        print('loading test data...')
        data, _ = load2d(test_data_file)
        names = np.load(test_names_file)
        print('generating kaggle submission...')

        generate_submission(data, names, net10, submission_file, header_file, N=50)
